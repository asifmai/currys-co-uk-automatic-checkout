...
class PuppeteerHelper {
  ...
  launchBrowser(params = { debug: false }) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await context.overridePermissions('https://www.currys.co.uk', ['geolocation', 'notifications']);
  ...
  cleanObjectForSaveing(object) {
    for (const key in object) {
      if (typeof object[key] != 'object') {
        object[key] = object[key].replace(/\"/gi, '').trim().replace(/\n/gi, '').trim();
      }
    }
  ...
  async saveToCsv(fileName, object) {
    let headerRow = '';
    for (const key in object) {
      headerRow += `"${key.toUpperCase()}",`;
    }
    headerRow = headerRow.replace(/\,$/gi, '') + '\n';
